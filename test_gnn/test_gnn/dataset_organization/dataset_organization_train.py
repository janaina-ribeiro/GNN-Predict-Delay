...
DELAY_DIR = BASE_DIR / "datasets delay"
TRACE_DIR = BASE_DIR / "datasets traceroute"
OUTPUT_DIR = BASE_DIR / "datasets_generated"
...
def process_pair(pair_name, delay_file, trace_file):
    ...
    try:
        ...
        loading_methods = [
            ...
            lambda: pd.read_csv(trace_file, header=None, on_bad_lines='skip', engine='python'),
            lambda: pd.read_csv(trace_file, header=None, on_bad_lines='warn', engine='python'),
            lambda: pd.read_csv(trace_file, header=None, sep=',', quoting=1, engine='python'),
        ...
        for i, method in enumerate(loading_methods):
            try:
                ...
                if df_trace is not None and not df_trace.empty:
                    if i > 0:
                        print(f"  Warning: Used alternative loading method #{i+1} for {trace_file.name}")
                    break
        ...
        if df_trace is None or df_trace.empty:
            raise ValueError(f"Could not load file {trace_file.name} with any method")
        ...
        for idx, row in df_trace.iterrows():
            ...
            for col_idx in range(len(row)):
                if pd.notna(row.iloc[col_idx]) and str(row.iloc[col_idx]).strip().lower() == 'gateway':
                    gateway_col = col_idx
                    break
            ...
            while col_idx < len(row):
                ...
                hostname_val = row.iloc[col_idx + 1] if (col_idx + 1) < len(row) else None
                
                if (pd.isna(ip_val) or str(ip_val).strip() == '' or 
                    str(ip_val).strip().lower() == 'nan'):
                    break
                
                ip_clean = str(ip_val).strip().replace("'", "")
                hostname_clean = str(hostname_val).strip().replace("'", "") if pd.notna(hostname_val) else "No Hostname"
        ...
        merged["high_delay"] = (merged["Atraso"] > 60).astype(int)
        ...
        print(f"   Records identified with high delay: {merged['high_delay'].sum()}")
        ...
        if "Expected" in str(e) and "fields" in str(e):
            print(f"   Suggestion: File {trace_file.name} has inconsistent column format")
        return False
...
for delay_file in delay_files:
    ...
    if not trace_candidates:
        print(f"No traceroute file found for {pair_name} in {TRACE_DIR}")
        errors += 1
        continue
